...
MASTER_IP=""
for ip in `kubectl get pods -o wide | egrep 'redis-cluster.-0' | awk '{print $6}'`; do
...
ix=0
while `true`; do
	slave_ip=`kubectl get pods -o wide | egrep "redis-cluster${ix}-1" | awk '{print $6}'`
	[ "${slave_ip}" = "" ] && break
	kubectl exec ${MASTER0} -- redis-cli -h ${slave_ip} -c FLUSHDB
	kubectl exec ${MASTER0} -- redis-cli -h ${slave_ip} -c CLUSTER RESET
	master_ip=`kubectl get pods -o wide | egrep "redis-cluster${ix}-0" | awk '{print $6}'`
